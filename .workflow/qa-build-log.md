...
#### Sprint 20 Validation Rules

| Check | Input | Expected | Actual | Result |
|-------|-------|----------|--------|--------|
| notes > 2000 chars | `PATCH` with 2001-char notes | HTTP 400 `VALIDATION_ERROR`, `fields.notes` | HTTP 400, `{"fields":{"notes":"Notes must be at most 2000 characters"}}` | ✅ PASS |
| destinations item > 100 chars | `POST` with 101-char destination | HTTP 400 `VALIDATION_ERROR`, `fields.destinations` | HTTP 400, `{"fields":{"destinations":"Each destination must be at most 100 characters"}}` | ✅ PASS |

#### Database

| Check | Evidence | Result |
|-------|----------|--------|
| DB connected | Auth register/login succeeds (reads/writes users table); trip CRUD succeeds (reads/writes trips table) | ✅ PASS |
| No migration pending | Deploy Engineer confirmed all 10 migrations applied | ✅ PASS |

#### Error Rate

| Check | Result |
|-------|--------|
| 5xx errors observed | None — all test requests returned 2xx or expected 4xx |
...
Timestamp: 2026-03-10T12:57:00Z
Checks:
  - [x] App responds (GET https://localhost:3001/api/v1/health → 200)
  - [x] Auth works (POST /api/v1/auth/register → 201; POST /api/v1/auth/login → 200 with token)
  - [x] Key endpoints respond (trips CRUD, auth, health — all per api-contracts.md)
  - [x] No 5xx errors
  - [x] Database connected (auth + trips CRUD confirmed working)
  - [x] Config consistency: backend PORT=3001 matches staging deployment
  - [x] Config consistency: HTTPS/TLS certs exist, protocol consistent across stack
  - [x] Config consistency: CORS_ORIGIN=https://localhost:4173 matches frontend preview origin
  - [x] Config consistency: Docker Compose (not in use for staging) internally consistent
  - [x] Sprint 20: notes field present in POST/GET/PATCH trip responses
  - [x] Sprint 20: notes max-2000-char validation enforced (400 on overflow)
  - [x] Sprint 20: destinations max-100-char-per-item validation enforced (400 on overflow)
Result: PASS ✅
Deploy Verified: YES ✅
```
