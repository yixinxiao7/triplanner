...
| 011 | 43 | Add `notes TEXT NULL` to `activities` table | Alter Table | `20260530_011_add_activity_notes.js` | ✅ **Implemented** (2026-05-30, T-331) — **Manager PRE-APPROVED** in Sprint 43 plan. ADR-007 recorded. Verified on dev DB: applies + rolls back + re-applies cleanly (`notes` is `text`, `is_nullable=YES`). Backend suite 531/531. **Awaiting staging deploy by Deploy Engineer (T-334).** Staging-only this sprint; production deferred to Sprint 44. |
...
**Sprint:** 43
**Task:** T-331
**Status:** ✅ **Implemented (2026-05-30, T-331)** — **Manager PRE-APPROVED** in Sprint 43 planning (`active-sprint.md` §"Manager schema approval"). ADR-007 recorded in `architecture-decisions.md`. Migration file, validation (max 2000), sanitize wiring (POST `sanitizeFields` + PATCH pre-validate strip), model `SELECT`/insert, route POST insert + PATCH `UPDATABLE`, 8 backend tests, and api-contracts update all landed. **Verified on dev DB:** `npm run migrate` applies (Batch 3), `npm run migrate:rollback` drops the column cleanly, re-apply succeeds — `notes` is `text`, `is_nullable=YES`. Backend suite **531/531** green. **Awaiting staging deploy by Deploy Engineer (T-334).** **Staging-only this sprint** — production promotion deferred to Sprint 44.
