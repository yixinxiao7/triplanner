...
describe('GET /api/v1/trips/:tripId/flights', () => {
  ...
  it('happy path: returns list of flights', async () => {
    ...
    const res = await request(buildApp(), 'GET', '/api/v1/trips/trip-1/flights', null, AUTH);
  ...
  it('error path: returns 404 when trip not found', async () => {
    ...
    const res = await request(buildApp(), 'GET', '/api/v1/trips/nonexistent/flights', null, AUTH);

    expect(res.status).toBe(404);
  });

  it('error path: returns 401 without auth', async () => {
    const res = await request(buildApp(), 'GET', '/api/v1/trips/trip-1/flights', null);
...
describe('POST /api/v1/trips/:tripId/flights', () => {
  ...
  it('happy path: creates flight and returns 201', async () => {
    const res = await request(buildApp(), 'POST', '/api/v1/trips/trip-1/flights', {
      flight_number: 'AA100',
      airline: 'American Airlines',
      from_location: 'JFK',
      to_location: 'LAX',
...
describe('DELETE /api/v1/trips/:tripId/flights/:id', () => {
  ...
  it('happy path: deletes flight and returns 204', async () => {
    ...
    const res = await request(buildApp(), 'DELETE', '/api/v1/trips/trip-1/flights/flight-1', null, AUTH);
  ...
  it('error path: returns 404 when flight not found', async () => {
    ...
    const res = await request(buildApp(), 'DELETE', '/api/v1/trips/trip-1/flights/nonexistent', null, AUTH);
